...
describe("SharedStack", () => {
  ...
  it("should create Artifact Registry with Docker format and immutable tags", () => {
    ...
    expect(artifactRegistry[registryKey].repository_id).toBe("n-koborinai-me-web")
  ...
  it("should create global static IP with EXTERNAL type", () => {
    ...
    expect(globalAddress[addressKey].name).toBe("n-koborinai-me-global-ip")
  ...
  it("should create dev Serverless NEG pointing to Cloud Run service", () => {
    ...
    expect(negs[devNegKey].name).toBe("n-koborinai-me-dev-neg")
    expect(negs[devNegKey].network_endpoint_type).toBe("SERVERLESS")
    expect(negs[devNegKey].cloud_run).toBeDefined()
    expect(negs[devNegKey].cloud_run.service).toBe("n-koborinai-me-web-dev")
  })
  ...
  it("should create prod Serverless NEG pointing to Cloud Run service", () => {
    ...
    expect(negs[prodNegKey].name).toBe("n-koborinai-me-prod-neg")
    expect(negs[prodNegKey].network_endpoint_type).toBe("SERVERLESS")
    expect(negs[prodNegKey].cloud_run).toBeDefined()
    expect(negs[prodNegKey].cloud_run.service).toBe("n-koborinai-me-web-prod")
  })
  ...
  it("should create dev Backend Service with IAP enabled and noindex header", () => {
    ...
    expect(backendServices[devBackendKey].name).toBe("n-koborinai-me-dev-backend")
  ...
  it("should create prod Backend Service without IAP and with logging", () => {
    ...
    expect(backendServices[prodBackendKey].name).toBe("n-koborinai-me-prod-backend")
  ...
  it("should create Managed SSL Certificate with multi-domain", () => {
    ...
    expect(sslCerts[certKey].name).toBe("n-koborinai-me-cert")
    expect(sslCerts[certKey].managed).toBeDefined()
    expect(sslCerts[certKey].managed.domains).toContain("n-koborinai.me")
    expect(sslCerts[certKey].managed.domains).toContain("dev.n-koborinai.me")
  })
  ...
  it("should create URL Map with host-based routing", () => {
    ...
    expect(urlMaps[urlMapKey].name).toBe("n-koborinai-me-url-map")
    ...
    const prodRule = hostRules.find((rule: { hosts: string[] }) => rule.hosts.includes("n-koborinai.me"))
    const devRule = hostRules.find((rule: { hosts: string[] }) => rule.hosts.includes("dev.n-koborinai.me"))
  ...
  it("should create Target HTTPS Proxy", () => {
    ...
    expect(httpsProxies[proxyKey].name).toBe("n-koborinai-me-https-proxy")
  })
  ...
  it("should create Global Forwarding Rule with PREMIUM tier", () => {
    ...
    expect(forwardingRules[ruleKey].name).toBe("n-koborinai-me-forwarding-rule")
  ...
  it("should create DNS A record for root domain", () => {
    ...
    expect(dnsRecords[rootRecordKey].name).toBe("n-koborinai.me.")
  ...
  it("should create DNS A record for dev subdomain", () => {
    ...
    expect(dnsRecords[devRecordKey].name).toBe("dev.n-koborinai.me.")
  ...
  it("should reference existing DNS managed zone", () => {
    ...
    expect(dnsZones[zoneKey].name).toBe("n-koborinai-me")
  })
})
