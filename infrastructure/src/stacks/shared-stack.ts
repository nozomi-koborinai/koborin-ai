 ...
 * - DNS managed zone (n-koborinai-me)
...
export class SharedStack extends TerraformStack {
  constructor(scope: Construct, id: string) {
    ...
    new ArtifactRegistryRepository(this, "artifact-registry", {
      ...
      repositoryId: "n-koborinai-me-web",
      description: "Container images for n-koborinai.me web application (dev/prod)",
      format: "DOCKER",
      dockerConfig: {
        immutableTags: true,
      },
    ...
    const staticIp = new ComputeGlobalAddress(this, "global-ip", {
      project: config.projectId,
      name: "n-koborinai-me-global-ip",
      addressType: "EXTERNAL",
      ipVersion: "IPV4",
      description: "Static IP for n-koborinai.me HTTPS load balancer",
      dependsOn: apiServices,
    })
    ...
    const devNeg = new ComputeRegionNetworkEndpointGroup(this, "dev-neg", {
      project: config.projectId,
      region: "asia-northeast1",
      name: "n-koborinai-me-dev-neg",
      networkEndpointType: "SERVERLESS",
      cloudRun: {
        service: "n-koborinai-me-web-dev",
      },
      lifecycle: {
    ...
    const devBackend = new ComputeBackendService(this, "dev-backend", {
      project: config.projectId,
      name: "n-koborinai-me-dev-backend",
      protocol: "HTTP",
    ...
    const prodNeg = new ComputeRegionNetworkEndpointGroup(this, "prod-neg", {
      project: config.projectId,
      region: "asia-northeast1",
      name: "n-koborinai-me-prod-neg",
      networkEndpointType: "SERVERLESS",
      cloudRun: {
        service: "n-koborinai-me-web-prod",
      },
      lifecycle: {
        createBeforeDestroy: true,
      },
    ...
    const prodBackend = new ComputeBackendService(this, "prod-backend", {
      project: config.projectId,
      name: "n-koborinai-me-prod-backend",
      protocol: "HTTP",
      loadBalancingScheme: "EXTERNAL_MANAGED",
      timeoutSec: 30,
      backend: [
    ...
    const sslCert = new ComputeManagedSslCertificate(this, "managed-cert", {
      project: config.projectId,
      name: "n-koborinai-me-cert",
      managed: {
        domains: ["n-koborinai.me", "dev.n-koborinai.me"],
      },
      lifecycle: {
        createBeforeDestroy: true,
      },
    ...
    const urlMap = new ComputeUrlMap(this, "url-map", {
      project: config.projectId,
      name: "n-koborinai-me-url-map",
      description: "Routes traffic to dev/prod backends based on host header",
      defaultService: prodBackend.id,
      hostRule: [
        { hosts: ["n-koborinai.me"], pathMatcher: "prod-matcher" },
        { hosts: ["dev.n-koborinai.me"], pathMatcher: "dev-matcher" },
      ],
      pathMatcher: [
    ...
    const httpsProxy = new ComputeTargetHttpsProxy(this, "https-proxy", {
      project: config.projectId,
      name: "n-koborinai-me-https-proxy",
      urlMap: urlMap.id,
    ...
    new ComputeGlobalForwardingRule(this, "forwarding-rule", {
      project: config.projectId,
      name: "n-koborinai-me-forwarding-rule",
    ...
    // 2. IAM binding grants impersonation only to specific subject (nozomi-koborinai/n-koborinai-me)
    ...
    // - Running workflows from the n-koborinai-me repository
    ...
    // (nozomi-koborinai/n-koborinai-me). This is more restrictive than attribute-based
    ...
    new ServiceAccountIamMember(this, "github-wif-user", {
      ...
      member: `principal://iam.googleapis.com/projects/${config.projectNumber}/locations/global/workloadIdentityPools/${workloadIdentityPool.workloadIdentityPoolId}/subject/nozomi-koborinai/n-koborinai-me`,
    })
    ...
    // A record for root domain (n-koborinai.me → Static IP)
    ...
    // A record for dev subdomain (dev.n-koborinai.me → Static IP)
    new DnsRecordSet(this, "dns-dev-a", {
      project: config.projectId,
      managedZone: dnsZone.name,
      name: "dev.n-koborinai.me.",
      type: "A",
      ttl: 300,
      rrdatas: [staticIp.address],
    })
