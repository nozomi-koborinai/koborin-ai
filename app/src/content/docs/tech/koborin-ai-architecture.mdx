...
description: Building a personal site with Astro × Pulumi Go × Google Cloud
...
### Why I Use a Single GCP Project
...
![gcs](../../../assets/tech/koborin-ai-architecture/gcs.png)
...
### Initial Choice: CDK for Terraform
...
Let me explain the flow when a user accesses the site.
...
![dns](../../../assets/tech/koborin-ai-architecture/dns.png)
...
| State | Screenshot |
| --- | --- |
| `Dev access` | ![iap-1](../../../assets/tech/koborin-ai-architecture/iap-1.png) |
| `Dev access denied` | ![iap-2](../../../assets/tech/koborin-ai-architecture/iap-2.png) |
...
Let me explain which files create what resources.
...
Let me explain the application layer structure.
...
# Install Playwright dependencies for Chromium and image optimization tools
...
http {
  server {
    ...
    # UTF-8 charset for text files (fixes llms.txt encoding)
    charset utf-8;
    ...
    # OG images - serve WebP version when PNG/JPEG is requested
    ...
    # OG images - WebP files directly
    location /og/ {
            expires 7d;
            add_header Cache-Control "public";
    }

    # Favicon - cache for 1 month
...
| `/llms-about-me.txt`, `/llms-ja-about-me.txt` | About Me only |

Files are generated as static files at build time, so there's zero runtime overhead.

### llms.txt for a Personal Site
...
#### Current Approach: Label-Based Branching
...
- **llms.txt**: Publishing articles in a format that my future self (and LLMs) can reference
