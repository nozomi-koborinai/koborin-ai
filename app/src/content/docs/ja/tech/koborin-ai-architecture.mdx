...
## 1 つの Google Cloud project + 3 Stack の設計

### Google Cloud project を 1 つで運用している理由
...
| 状態 | イメージ |
| --- | --- |
| アクセス時 | ![iap-1](../../../../assets/tech/koborin-ai-architecture/iap-1.png) |
| アクセス拒否時 | ![iap-2](../../../../assets/tech/koborin-ai-architecture/iap-2.png) |

さらに、dev 環境のレスポンスには `X-Robots-Tag: noindex, nofollow` ヘッダーを付与しているため、万が一アクセスできたとしても検索エンジンにはインデックスされません。

## Stack 別インフラ実装

どのファイルが何を作っているかを紹介します。
...
| `/llms-about-me.txt`, `/llms-ja-about-me.txt` | About Me のみ |
...
### About Me
- https://koborin.ai/llms-about-me.txt
- https://koborin.ai/llms-ja-about-me.txt
```
...
- **llms.txt**: 将来の自分（や LLM）が参照できる形式で記事を公開
